...
int main(int argc, const char **argv)
{
	...
	try {
        ...
        shared_ptr<sql::ResultSet> res(stmt->executeQuery("select Concat(vend_name, ' (', vend_country, ')') from vendors order by vend_name;"));
        ...
        "select order_num, sum(quantity * item_price) as order_price from orderitems group by order_num having order_num = ?;"));
        pst->setInt(1, 20005);
        shared_ptr<sql::ResultSet> res2(pst->executeQuery());
        cout<<"Row count2: "<< res2->rowsCount()<<endl;
        while(res2->next()) {
            cout<<res2->getInt(1)<<", "<<res2->getDouble(2)<<endl;
        }
